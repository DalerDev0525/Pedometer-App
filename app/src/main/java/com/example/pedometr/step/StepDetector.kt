...
class StepDetector {
    ...
    fun updateAccel(timeNs: Long, x: Float, y: Float, z: Float) {
        ...
        worldZ[0] = SensorFilter.sum(accelRingX) / accelRingCounter.coerceAtMost(ACCEL_RING_SIZE)
        worldZ[1] = SensorFilter.sum(accelRingY) / accelRingCounter.coerceAtMost(ACCEL_RING_SIZE)
        worldZ[2] = SensorFilter.sum(accelRingZ) / accelRingCounter.coerceAtMost(ACCEL_RING_SIZE)
